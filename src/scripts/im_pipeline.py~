...
def runcmds(cmd_templates, outputfilenames, argdicts, jobname, outputfile_dir=None):
    assert len(cmd_templates) == len(outputfilenames) == len(argdicts)
    num_cmds = len(cmd_templates)
    outputfile_dir = outputfile_dir if outputfile_dir is not None else 'logs_%s_%s' % (jobname, datetime.datetime.now().strftime('%Y-%m-%d_%H:%M:%S'))
    ...
    if raw_input() == 'y':
        ...
        for i in xrange(num_cmds):
            print 'process', cmds[i]
            subprocess.check_call(cmds[i], shell=True)
    else:
        raise RuntimeError('Canceled.')
            ...
            for num_trajs in spec['training']['dataset_num_trajs']:
                ...
                for run in range(spec['training']['runs']):
                    ...
                    if spec['training']['use_gpu']:
                        cmd_templates.append(gpu_cmd_prefix + ' ' + alg['cmd'].replace('\n', ' ').strip())
                    else:
...
#    )
    runcmds(
        ...
        outputfile_dir=os.path.join(checkptdir, 'logs_%s_%s' % (pbsopts['jobname'], datetime.datetime.now().strftime('%Y-%m-%d_%H:%M:%S')))
    )
...
def main():
    ...
    parser.add_argument('--phase', type=str, choices=sorted(phases.keys()))
