...
#    print 'Debug: optim: ngstep'
#    print 'Debug: obj0 type:', type(obj0)
#    print 'Debug: maxk_kl type:', type(max_kl)
#    print 'Debug: damping type:', type(damping)
    if theano.config.floatX == 'float32':
        obj0 = np.float32(obj0)
        max_kl = np.float32(max_kl)
...
#    print 'Debug: damping type:', type(damping)
    ...
    hvpop = ssl.LinearOperator(shape=(x0.shape[0], x0.shape[0]), matvec=damped_hvp_func, dtype=theano.config.floatX)
#    print 'Debug: hvpop type:', type(hvpop)
    step, _ = ssl.cg(hvpop, -objgrad0, maxiter=max_cg_iter)
#    print 'Debug: step type:', type(step)
    fullstep = step / np.sqrt(.5 * step.dot(damped_hvp_func(step)) / max_kl + 1e-8)
#    print 'Debug: fullstep type:', type(fullstep)
...
#    print 'Debug: xnew type:', type(xnew)
#    print 'Debug: num_bt_stpes:', type(num_bt_steps)
...
def make_ngstep_func(model, compute_obj_kl, compute_obj_kl_with_grad, compute_kl_hvp):
    ...
    def wrapper(feed, max_kl, damping, subsample_hvp_frac=.1, grad_stop_tol=1e-6, max_cg_iter=10, enable_bt=True, sequential_model=False):
        assert isinstance(feed, tuple)

#        print 'Debug: optim: wrapper'
#        print 'Debug: max_kl type:', type(max_kl)    # float
#        print 'Debug: damping type:', type(damping)    # float
        params0 = model.get_params()
        obj0, kl0, objgrad0 = compute_obj_kl_with_grad(*feed)
#        print 'Debug: params0 type:', type(params0)    # numpy.ndarray
#        print 'Debug: params0 dtype:', params0.dtype    # float32
#        print 'Debug: obj0 type:', type(obj0)    # float
#        print 'Debug: kl0 type:', type(kl0)    # float
#        print 'Debug: objgrad0 type:', type(objgrad0)    # numpy.ndarray
#        print 'Debug: objgrad0 dtype:', objgrad0.dtype    # float32
        gnorm = util.maxnorm(objgrad0)
#        print 'Debug: gnorm type:', type(gnorm)    # numpy.float32
        # XXX not check initial kl in sequential model
        # XXX not check initial kl by now !!!!! (check that later)
...
#            assert np.allclose(kl0, 0), 'Initial KL divergence is %.7f, but should be 0' % (kl0,)
        ...
        params1, num_bt_steps = ngstep(
            ...
            enable_bt=enable_bt)
#        print 'Debug: params1 type:', type(params1)
#        print 'Debug: num_bt_steps:', type(num_bt_steps)
